...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  <div className="relative w-full h-full">
    ...
    {activeItem && (
      <>
        {/* TEXT BLOCK: centered below the (slightly-above-center) image */}
        <div
          className={`
            absolute
            left-1/2 top-[45%] -translate-x-1/2
            w-[92%] max-w-[46rem] px-4 py-6
            text-center
            rounded-2xl
            bg-black/30
            backdrop-blur-md
            ...
            ${isMoving
              ? 'opacity-0 pointer-events-none translate-y-4 duration-[100ms]'
              : 'opacity-100 pointer-events-auto translate-y-0 duration-[500ms]'}
          `}
        >
          <h2 className="font-black text-3xl sm:text-5xl md:text-6xl mb-3">
            {activeItem.title}
          </h2>
          ...
          {activeItem.link && (
            <a
              ...
              rel={activeItem.link.startsWith('http') ? 'noopener noreferrer' : undefined}
              className="inline-block mt-4 sm:mt-5 text-base sm:text-lg font-semibold underline underline-offset-4 decoration-white/40 hover:decoration-white focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-white/60 rounded"
            >
              Visit here
            </a>
          )}
